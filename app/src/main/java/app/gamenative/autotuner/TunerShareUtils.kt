...
object TunerShareUtils {
    ...
    fun buildSharePayload(
        ...
        val issueBody = buildString {
            ...
            appendLine("```")
            appendLine()
            appendLine("*Generated by the NovaGN auto-tuner — results are empirical but reflect menu/early-game only.*")
        }
    ...
    fun shareViaIntent(context: Context, payload: SharePayload) {
        val intent = Intent(Intent.ACTION_SEND).apply {
            type = "text/plain"
            putExtra(Intent.EXTRA_SUBJECT, "NovaGN tuner result: ${payload.gameName} on ${payload.gpuName}")
