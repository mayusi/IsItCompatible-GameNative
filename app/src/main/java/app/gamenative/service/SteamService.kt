...
class SteamService : Service(), IChallengeUrlChanged {
    ...
    companion object {
         ...
         * user picked in settings (SD card / USB). On modern we use the configured
         * externalStoragePath (which should be a real removable volume path).
         * Falls back to the configured path; the FUSE primary-external path
         * (/storage/emulated/0/Android/data/<pkg>) is intentionally NOT used here
         * because it is FUSE-backed and kills game processes at launch (FuseDaemon crash).
         */
        ...
        val allInstallPaths: List<String>
            get() {
                ...
                // Also include the legacy FUSE primary-external path so we can still
                // *find* games that were previously installed there (for migration).
                if (BuildConfig.MODERN_ANDROID && DownloadService.baseExternalAppDirPath.isNotBlank()) {
                    val fuseCommon = Paths.get(
        ...
        /**
         * True only when "use external storage" is on AND a real non-primary removable
         * volume (SD card / USB OTG) is mounted.
         *
         * The FUSE-backed primary-external dir (/storage/emulated/0/Android/data/<pkg>)
         * is NOT counted as "ready" for game installs.  When Wine/Box64 rapidly opens
         * dozens of game files on that FUSE mount, Android's FuseDaemon
         * (MediaProvider/pf_opendir) crashes and vold tears down the mount, sending
         * SIGKILL to any process holding open fds on that volume — i.e. the game process.
         * Games on the real ext4/f2fs internal partition (/data/data/<pkg>) do NOT have
         * this problem.  External storage is therefore only usable for game execution
         * when a genuine removable volume is present.
         */
        private val externalStorageReady: Boolean
            get() = PrefManager.useExternalStorage && DownloadService.externalVolumePaths.isNotEmpty() &&
                File(externalAppInstallRoot).let { it.path.isNotBlank() && it.exists() }
        ...
        val defaultAppInstallPath: String
            get() {
                return if (externalStorageReady) {
                    Timber.i("Using external storage (real removable volume)")
                    Timber.i("install path for external storage is $externalAppInstallPath")
                    externalAppInstallPath
                } else {
                    if (!externalStorageReady && PrefManager.useExternalStorage) {
                        Timber.w(
                            "External storage requested but no real removable volume detected " +
                                "(FUSE primary-external is not safe for game executables). " +
                                "Routing new install to internal storage.",
                        )
                    }
        ...
        fun getAppDirPath(gameId: Int): String {
            ...
            // Only route to external if a genuine removable volume is available
            // (externalStorageReady enforces this — FUSE primary-external is excluded).
