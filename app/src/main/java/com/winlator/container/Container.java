...
public class Container {
    ...
    private String suspendPolicy = SUSPEND_POLICY_AUTO;
    ...
    public static void checkObsoleteOrMissingProperties(JSONObject data) {
        try {
            ...
            if (data.has("dxwrapper")) {
                ...
                // Self-heal: if dxwrapper is "dxvk" and dxwrapperConfig is missing the
                // version= key, inject the default DXVK version so that launch code never
                // builds the invalid wrapper name "dxvk-" (empty version).
                if (dxwrapper.equals("dxvk")) {
                    String cfg = data.optString("dxwrapperConfig", "");
                    KeyValueSet cfgSet = new KeyValueSet(cfg);
                    String ver = cfgSet.get("version");
                    if (ver.isEmpty()) {
                        cfgSet.put("version", DefaultVersion.DXVK);
                        data.put("dxwrapperConfig", cfgSet.toString());
                    }
                }
            }
            ...
            // Self-heal: if suspendPolicy is "manual", upgrade it to "auto" so existing
            // containers don't have the stuck paused game with untappable resume button.
            if (data.has("suspendPolicy")) {
                String suspendPolicy = data.optString("suspendPolicy", "");
                if (suspendPolicy.equals(SUSPEND_POLICY_MANUAL)) {
                    data.put("suspendPolicy", SUSPEND_POLICY_AUTO);
                }
            }
