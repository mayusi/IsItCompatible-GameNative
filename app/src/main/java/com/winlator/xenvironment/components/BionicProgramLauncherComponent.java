...
public class BionicProgramLauncherComponent extends GuestProgramLauncherComponent {
    ...
    private int execGuestProgram() {
        ...
        // Pre-create the gamepad shared-memory files in the correct location:
        // <app-files-dir>/gamepad_shm/gamepad.mem (and gamepad1.mem etc. for extra players).
        // This must match EVSHIM_BASE_PATH set below so the native shim finds the same files
        // that WinHandler.java writes.
        ...
        for (int i = 0; i < enabledPlayerCount; i++) {
            ...
            File memFile = new File(gamepadShmDir, filename);
        ...
        // Without this it falls back to /data/data/app.gamenative/files (wrong package name).
